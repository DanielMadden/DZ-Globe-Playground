...
function loadScriptOnce(src: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    s.src = src;
    s.async = true; // do NOT set type="module" (we want UMD)
    s.onload = () => resolve();
...
// Try UMD builds that attach window.Globe
...
export default function GlobeCDN({
  ...
  React.useEffect(() => {
    if (didInitRef.current) return; // skip the second dev-invoke
    ...
    (async () => {
      // 1) Load UMD so window.Globe exists
      ...
      return () => {
        cancelled = true;
        clearAllTimers(); // <-- important
        try {
          // dispose globe/three resources if present
          ...
          if (globe) {
            ...
            // dispose geometries/materials from scene
            ...
            renderer?.dispose?.();
            // Try to force the GL context to release:
            try {
              renderer
                ?.getContext?.()
  ...
  React.useEffect(() => {
    ...
    // If the accessor is cached per data, a nudge helps:
...
// NOTE the extra logo spin functionality is in SpareCode.tsx
